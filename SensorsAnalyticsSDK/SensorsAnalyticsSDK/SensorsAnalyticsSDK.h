...
/**
 *  所部署的SensorsAnalytics服务器的地址
 */
@property (atomic, readonly, strong) NSString *serverURL;
 ...
 * flush的周期，单位是秒
 */
@property (atomic, readonly) NSUInteger flushInterval;
...
/**
 *  @abstract
 *  根据传入的所部署的SensorsAnalytics服务器的URL和刷新时间，返回一个<code>SensorsAnalyticsSDK</code>的单例。
 *  同时，也会默认track一个名为$appStart的事件
 *
 *  @discussion
 *  flushInterval是指两次发送的最小间隔
 *  在每次调用track、signUp以及profileSet等接口的时候，都会检查如下条件，以判断是否向服务器上传数据:
 *  1. 当前是否是WIFI/3G/4G网络条件
 *  2. 与上次发送的时间间隔是否大于flushInterval
 *  如果满足这两个条件，则向服务器发送一次数据；如果不满足，则把数据加入到队列中，等待下次检查时把整个队列的内容一并发送。需要注意的是，为了避免占用过多存储，队列最多只缓存1000条数据。
 *
 *  @param serverURL     传入的URL
 *  @param flushInterval 刷新周期，单位是秒
 *
 *  @return 返回的单例
 */
 ...
 *
 *  @discussion
 *  当track的Properties，superProperties和SDK自动生成的automaticProperties有相同的key时，遵循如下的优先级：track.properties > superProperties > automaticProperties
 ...
 *  同时，SDK会自动将superProperties保存到文件中，下次启动时也会从中读取
 *
 *
